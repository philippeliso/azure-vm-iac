...
resource "azurerm_virtual_machine" "az-eus-nix-prod-webcache1" {
  name                  = "az-eus-nix-prod-webcache1"
  location              = "East US"
  resource_group_name   = "${azurerm_resource_group.az-eus-rs-prod.name}"
  network_interface_ids = ["${azurerm_network_interface.az-eus-if-webcache1.id}"]
  ...
  storage_os_disk {
    name              = "az-eus-os-disk-prod-webcache1"
    caching           = "ReadWrite"
  ...
  os_profile {
    computer_name  = "az-eus-nix-prod-webcache1"
